...
% rjprill@us.ibm.com
% Small changes made by Nitin Singh, UT Dallas, 2014 Jan

function [auroc, aupr, score] = eval_dream5_nws(datapath, dataname)
    addpath(genpath('./'));
    %% gold standard edges only
    nw = [datapath, '/', dataname];
    goldfile = [nw, '_gold_standard.txt'];
    %% predicted edges
    results_dir = [nw, '_result/'];
    predictionfile = [results_dir, 'arni_prediction.txt'];

    %% precomputed probability densities for various metrics
    pdffile_aupr  = [nw, '_AUPR.mat'];
    pdffile_auroc = [nw, '_AUROC.mat'];
...
%     subplot(2,2,1)
%     plot(fpr,tpr)
%     title(['AUROC: ', num2str(auroc), ' p-value: ', num2str(p_auroc)]);
%     xlabel('FPR')
%     ylabel('TPR')
%     subplot(2,2,2)
%     plot(rec,prec)
%     title(['AUPR: ', num2str(aupr), ' p-value: ', num2str(p_aupr)]);
%     xlabel('Recall')
%     ylabel('Precision')
end
